    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (resultResponse.data.status === 1) {
          ...
          resultResponse.data.request.includes("ERROR_CAPTCHA_UNSOLVABLE")
        ) {
          console.error("2Captcha reports unsolvable captcha.  Aborting.");
          return null;
        } else {
          console.warn(
            `2Captcha error: ${resultResponse.data.request}. Retrying...`
          );
          delay = Math.min(delay * 2, 2000);
          continue;
        }
        ...
        if (axiosError.code === "ECONNABORTED") {
          console.error(
            `Timeout checking captcha solution (attempt ${attempts}). Retrying...`
          );
        } else {
          console.error(
...
async function injectTurnstileToken(page, token) {
  await page.evaluate((token) => {
    ...
    document
      ...
      .forEach((el) => el.setAttribute("data-token", token));
  }, token);
}
...
async function processCedula(cedula, apiKey, browser, cedulaTimeout) {
  ...
  try {
    ...
    await page.type(inputSelector, cedula);
    ...
    if (siteKey) {
      console.log(
        `Cedula ${cedula} – Found Turnstile site key: ${siteKey}. Attempting to solve...`
      );
      ...
      if (token) {
        console.log(
          `Cedula ${cedula} – Successfully obtained token. Injecting...`
        );
        ...
        return {
          ...
          primerNombre: "ERROR",
          segundoNombre: "ERROR",
          estado: "Error: Captcha unsolved",
        };
      }
    } else {
      console.warn(
        `Cedula ${cedula} – Turnstile site key not found. Continuing without token.`
      );
    }
    ...
    try {
      ...
      console.error(
        `Cedula ${cedula} – No results found after waiting: ${error.message}`
      );
      return {
        cedula,
        primerApellido: "ERROR",
    ...
    console.log(
      `Cedula ${cedula} – Execution time: ${endTimeCedula - startTimeCedula} ms`
    );
  }
}
...
async function scrapeDianCedulas(
  ...
  if (balance === null || balance <= 0) {
    console.error(
      "ERROR: Your 2Captcha account has no funds or there was an error checking the balance. Please add credit before proceeding."
    );
    return;
  }
  ...
  try {
    const tasks = cedulas.map(
      (cedula) => () => processCedula(cedula, apiKey, browser)
    );
    ...
    const header =
      "cedula,primer apellido,segundo apellido,primer nombre,segundo nombre,estado\n";
    const csvData = resultados
      .map(
        (r) =>
          `${r.cedula},${r.primerApellido},${r.segundoApellido},${r.primerNombre},${r.segundoNombre},${r.estado}`
      )
      .join("\n");
    ...
    console.log(
      "Process completed. Results saved to dian_resultados.csv and dian_resultados.json"
    );
    return resultados;
  } catch (error) {
    ...
    console.log(
      `Total execution time: ${endTimeOverall - startTimeOverall} ms`
    );
  }
}
...
// --- PRINCIPAL ---
async function main() {
  const apiKey = "6b839fc1d6dd5a9a77261a4fdc2aeb1f"; //  Tu API key de 2Captcha
  const excelFilePath = "./test.xlsx";
  const concurrency = 100;
  const timeout = 15 * 60 * 500;
